<?xml version="1.0" encoding="UTF-8"?>
<tileset version="1.10" tiledversion="1.11.0" name="hazard" tilewidth="32" tileheight="32" tilecount="8" columns="4">
 <image source="../../../images/objects_hazard.png" width="128" height="64"/>
 <tile id="0">
  <properties>
   <property name="hazard" type="bool" value="true"/>
   <property name="height" type="int" value="8"/>
   <property name="hitbox" type="bool" value="true"/>
   <property name="offsetX" type="int" value="24"/>
   <property name="offsetY" type="int" value="2"/>
   <property name="width" type="int" value="28"/>
  </properties>
 </tile>
 <tile id="1">
  <properties>
   <property name="hazard" type="bool" value="true"/>
   <property name="height" type="int" value="0"/>
   <property name="hitbox" type="bool" value="true"/>
   <property name="offsetX" type="int" value="0"/>
   <property name="offsetY" type="int" value="0"/>
   <property name="width" type="int" value="0"/>
  </properties>
 </tile>
 <tile id="2">
  <properties>
   <property name="hazard" type="bool" value="true"/>
   <property name="height" type="int" value="0"/>
   <property name="hitbox" type="bool" value="true"/>
   <property name="offsetX" type="int" value="0"/>
   <property name="offsetY" type="int" value="0"/>
   <property name="width" type="int" value="0"/>
  </properties>
 </tile>
 <tile id="3">
  <properties>
   <property name="hazard" type="bool" value="true"/>
   <property name="height" type="int" value="0"/>
   <property name="hitbox" type="bool" value="true"/>
   <property name="offsetX" type="int" value="0"/>
   <property name="offsetY" type="int" value="0"/>
   <property name="width" type="int" value="0"/>
  </properties>
 </tile>
 <tile id="4">
  <properties>
   <property name="hazard" type="bool" value="true"/>
   <property name="height" type="int" value="0"/>
   <property name="hitbox" type="bool" value="true"/>
   <property name="offsetX" type="int" value="0"/>
   <property name="offsetY" type="int" value="0"/>
   <property name="width" type="int" value="0"/>
  </properties>
 </tile>
 <tile id="5">
  <properties>
   <property name="hazard" type="bool" value="true"/>
   <property name="height" type="int" value="0"/>
   <property name="hitbox" type="bool" value="true"/>
   <property name="offsetX" type="int" value="0"/>
   <property name="offsetY" type="int" value="0"/>
   <property name="width" type="int" value="0"/>
  </properties>
 </tile>
 <tile id="6">
  <properties>
   <property name="hazard" type="bool" value="true"/>
   <property name="height" type="int" value="0"/>
   <property name="hitbox" type="bool" value="false"/>
   <property name="offsetX" type="int" value="0"/>
   <property name="offsetY" type="int" value="0"/>
   <property name="width" type="int" value="0"/>
  </properties>
 </tile>
 <tile id="7">
  <properties>
   <property name="hazard" type="bool" value="true"/>
   <property name="height" type="int" value="0"/>
   <property name="hitbox" type="bool" value="false"/>
   <property name="offsetX" type="int" value="0"/>
   <property name="offsetY" type="int" value="0"/>
   <property name="width" type="int" value="0"/>
  </properties>
 </tile>
</tileset>
